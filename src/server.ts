...
const SERVER_INSTRUCTIONS = [
    ...
    'Refine and forge resources with `cpu_craft` — discover recipes via `cpu_list_recipes` (the `forge_wcpu` recipe',
    'is paid in $CPU and auto-settled on-chain, the rest are free); read progress with `cpu_get_craft_status` and',
    'bank matured batches with `cpu_claim_craft`.',
