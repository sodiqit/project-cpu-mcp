...
const SERVER_INSTRUCTIONS = [
    ...
    '`finalize_delivery` on the returned deliveryId once it arrives; track your lots with `list_my_lots`.',
    'Check spendable $CPU and gas with `get_balance` before any paid action.',
    'A `withdraw` mints $CPU against an off-chain signature; if it is interrupted, re-run it with the same args',
