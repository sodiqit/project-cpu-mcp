export const GET_MAP_DESCRIPTION = [
    ...
    'owned/other/empty, empty = unminted) or `cpu_plan_route`; `pos {face,i,j}` is only a rough hint (wraps at',
    'face seams). Returns a `summary`, a `resourceIndex` of your cells, and per-resource `storage`',
    '(used/cap/stalled; a full box halts that resource’s production, cap null = uncapped). Scopes: mine | around',
