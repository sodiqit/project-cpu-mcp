export const GET_ATTENTION_DESCRIPTION = [
    ...
    'stalled mining/craft (a warehouse hit its cap and',
    ...
    'a one-line action. Optional `minSeverity` filters by urgency. Pass `owner` to scout another player',
    'instead — read-only intel on their cells (who is stalled, near-full, sitting on idle land, or has goods',
    'inbound); the suggested actions then describe what that owner would do. Everything here is public.',
    'Reads the in-memory map plus the owner’s ready deliveries; if the server is unreachable the delivery',
    'section is skipped and a `note` says so.',
].join(' ');
