export const BUILD_DESCRIPTION = [
    'Place a building on a revealed Land cell you own. Requires a session — call `cpu_authenticate` first.',
    ...
    'transaction — track it with `cpu_get_mining_status`. The new state appears on the map shortly after; inspect',
    'it with `cpu_get_cell`.',
    'A cell holds one building; to change to a different building, `cpu_demolish` it first. Re-running build on a',
