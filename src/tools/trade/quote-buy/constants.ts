export const QUOTE_BUY_DESCRIPTION = [
    ...
    'session. Pass `chain` = [hub, ...waypoints, your destination cell] for the exact total `cpu_buy_lot` would charge',
    ...
    '`transitFee`, summed as `total` — the expected charge. `cpu_buy_lot` re-quotes transit on-chain, so the transit',
    'part can move slightly; it authorizes ~10% over the quoted fee as headroom for that drift — a ceiling, not an',
    'extra charge (the contract debits the real fee). Use before `cpu_buy_lot`.',
].join(' ');
