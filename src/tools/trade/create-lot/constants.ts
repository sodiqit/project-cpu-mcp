export const CREATE_LOT_DESCRIPTION = [
    'List units of a resource for sale at a Hub, on-chain. Requires a session — call `cpu_authenticate` first.',
    ...
    'and becomes buyable (OPEN) only after its escrow arrives — after arrivalAt, call `cpu_finalize_delivery` on the',
    'returned deliveryId (or wait for someone to). Returns the lotId — track it with `cpu_list_my_lots` / `cpu_get_lot`.',
].join(' ');
