export const START_MINING_DESCRIPTION = [
    ...
    'the current run with `cpu_claim_mining` first, then start again. The extractor mines a batch each cycle —',
    'track matured batches with `cpu_get_mining_status` and bank them with `cpu_claim_mining`.',
].join(' ');
