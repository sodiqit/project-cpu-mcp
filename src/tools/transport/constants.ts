export const TRANSPORT_DESCRIPTION = [
    ...
    'revealed and yours-or-a-Hub; each hop must be within radius(from)+radius(to) grid steps (default balance:',
    'own↔own 2, own↔hub 4, hub↔hub 6). Recommended flow:',
...
export const ROUTE_NETWORK_DESCRIPTION = [
    ...
    'would accept as edges (within radius(a)+radius(b) grid steps; default balance: own↔own 2, own↔hub 4,',
    'hub↔hub 6). Foreign cells are never nodes: a belt of foreign land wider than the reach with no hubs is a',
    'WALL, and different component ids show exactly that — no chain crosses today, goods stop at the border.',
...
export const NEXT_HOPS_DESCRIPTION = [
    ...
    'and all Hubs — within one hop under the on-chain reach rule: hop ≤ radius(from)+radius(to) grid steps',
    '(default balance: own↔own 2, own↔hub 4, hub↔hub 6). Foreign cells are never waypoints, so a belt of',
    'foreign land wider than your reach with no hubs is a WALL: empty `hops` means goods can only be hauled up',
    'to here. Facts per candidate: hop distance, owner, hub flag, per-unit transit fee, and — with `towards` —',
