export const TRANSPORT_DESCRIPTION = [
    'Move a resource between cells along a waypoint chain, on-chain. Requires a session — call `cpu_authenticate` first.',
    ...
    'you also pay gas. Preview the fee and ETA first with `cpu_quote_transport`. Returns the on-chain deliveryId and',
    'arrival time. The delivery is not credited to the target until it arrives and is finalized — after arrivalAt,',
    'call `cpu_finalize_delivery` (`cpu_list_my_transports` shows what is ready). A route over only your own cells pays no',
    '$CPU fee (just gas).',
].join(' ');
