#!/usr/bin/env node
// Regenerates src/geometry/adjacency.data.ts — the neighbor table of the game's world grid
// (a Goldberg GP(70,0) sphere: 10 rhombus faces of 70×70 lattice points, 48 990 hex cells with
// tokenIds 1..48990 plus 12 pentagon vertices that carry no tokenId and never appear as neighbors).
// The table must match the deployed contracts' grid; the invariant tests in
// src/geometry/__tests__/ re-verify the committed blob on every run.
//
...
// --- lattice → cells (seam dedup by rounded unit-sphere position) --------------------------------
...
// --- sanity: degree histogram, symmetry, connectivity --------------------------------------------
...
// GENERATED by scripts/generate-adjacency.mjs — do not edit by hand.
// Neighbor table of the game's world grid (Goldberg GP(70,0)): 48990 cells × 6 uint16 slots,
// little-endian, base64-packed; slot value 0 = no neighbor (the 60 cells adjacent to a pentagon
// vertex have 5 neighbors). Must match the deployed contracts' grid.
`;

const body = `export const ADJACENCY_BASE64: string = [\n${chunks.map((c) => `    '${c}',`).join('\n')}\n].join('');\n`;
